...
public class MediaManagerTests
{
    ...
    public void GenerateFileLinks_ShouldReplaceTemplateWithLinksInText()
    {
        ...
        actual.Should()
            .Be($"text1 text [link]({staticUrl}/random-path/file.pdf) text text text" +
    ...
    public async Task SaveFilesAsync_ShouldReturnSavedFilesWithRandomPath()
    {
        ...
        _fileServiceMock.Verify(x => x.SaveFileAsync(It.IsIn(files.AsEnumerable()), It.IsAny<string>()),
    ...
    public async Task SaveImagesAsync_ShouldOptimizeAndReturnFilesWithRandomPath()
    {
        ...
        _fileServiceMock.Verify(x => x.DownloadFileAsync(It.IsAny<Uri>(), It.IsAny<string>()),
            Times.Exactly(files.Count));

        _imageOptimizerMock.Verify(
            x => x.OptimizeAsync(It.IsIn(files.AsEnumerable()), It.IsAny<string>(), It.IsAny<bool>()),
            Times.Exactly(files.Count));
    }
}